...
def summarize(observations: Iterable[Observation], policy: OutlierPolicy = OutlierPolicy()) -> StatisticSummary:
    raw = tuple(observations)
    valid = tuple(o.value_mm for o in raw if o.status is not SampleStatus.INVALID and o.acceptance is AcceptanceStatus.ACCEPTED)
    ...
    mad = median(tuple(abs(v - med) for v in filtered)) if n and med is not None else None
    ...
    drift = (raw[-1].value_mm - raw[0].value_mm) if len(raw) >= 2 and all(o.status is not SampleStatus.INVALID for o in (raw[0], raw[-1])) else None
    return StatisticSummary(raw, ordered, filtered, len(raw), len(valid), sum(o.status is SampleStatus.INVALID for o in raw), warning,
                            rejected, mean, med, sd, mad, min(filtered) if n else None, max(filtered) if n else None,
                            (max(filtered) - min(filtered)) if n else None, uncertainty, drift, n < 2, policy)
