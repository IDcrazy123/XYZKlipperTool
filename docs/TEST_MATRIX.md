...
| TEST-031 | Real JPEG host exploratory adapter | `PYTHONPATH=src; python -m unittest tests.test_jpeg_adapter -v`; explicit `python scripts/explore_picture.py <path> --calibration-id ... --camera-fingerprint ... --source-sha256 ... --roi X Y WIDTH HEIGHT --mm-per-pixel-x ... --mm-per-pixel-y ...` | both bounded pipelines, quality diagnostics, identity/input faults pass; user images remain unlabeled/excluded and no metric claim |
