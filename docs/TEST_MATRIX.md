...
| TEST-028 | Phase 03 committed synthetic corpus | `pinned venv; PYTHONPATH=src; python -m unittest tests.test_phase03.Phase03Tests.test_committed_synthetic_fixture_and_benchmark_artifact_validate` | four-session PGM fixture hashes, labels, relative paths, manifest and benchmark reports validate |
| TEST-029 | Phase 03 actual detector holdout recomputation | `pinned venv; PYTHONPATH=src; python -m unittest tests.test_phase03.Phase03Tests.test_committed_synthetic_fixture_and_benchmark_artifact_validate` | Blob/Circle run on committed HOLDOUT; artifact exactly matches derived TP/FP/FN/TN and failure metrics |
| TEST-030 | Phase 03 HIL partial evidence recovery | `Get-FileHash -Algorithm SHA256` plus metadata reconciliation over `evidence/hil/phase-03`; secret, Markdown, link, and parity checks | 4 raw JPEGs reconcile; 1 invalid + 3 excluded LED-on; 0 T2; HIL remains blocked |
| TEST-031 | Real JPEG host exploratory adapter | `PYTHONPATH=src; python -m unittest tests.test_jpeg_adapter tests.test_archive_pixel_diagnostics -v`; `python scripts/archive_pixel_diagnostics.py --source-root ROOT --source-manifest MANIFEST --output NEW_DIR --roi 620 230 220 240` | versioned `frames[]`/HIL `items[]`/capture-library `photos[]` ingestion; source status preserved; hash/byte/dimension/time/name bounds; preaccepted/ambiguous/duplicate/traversal/collision/oversize/ROI faults fail closed |
| TEST-032 | Real acquisition replay and unlabeled holdout gate | verify both capture manifests against 75 JPEGs; run archive inspector into `90_TESTS_ANALYSIS/20260905_Phase03_PixelDiagnostics_75frames_v1`; `PYTHONPATH=src; python -m unittest tests.test_phase03.Phase03Tests.test_benchmark_rejects_unlabeled_or_incomplete_ground_truth` | 75/75 byte/hash matches, 75 `WARNING`/excluded, 150 overlays, 0 accepted/machine-eligible; unlabeled or positive-without-center benchmark input is rejected |
