...
| TEST-031 | Real JPEG host exploratory adapter | `PYTHONPATH=src; python -m unittest tests.test_jpeg_adapter -v`; explicit `python scripts/explore_picture.py <path> --roi X Y WIDTH HEIGHT` for pixel-only archive diagnostics, or add all explicit calibration arguments to run both pipelines | bounded pixel diagnostics always; detector pipelines only with supplied calibration; user images remain unlabeled/excluded and no metric claim |
