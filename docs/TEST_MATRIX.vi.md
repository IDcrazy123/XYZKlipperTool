...
| TEST-031 | Adapter exploratory JPEG thật phía host | `PYTHONPATH=src; python -m unittest tests.test_jpeg_adapter tests.test_archive_pixel_diagnostics -v`; `python scripts/archive_pixel_diagnostics.py --source-root ROOT --source-manifest MANIFEST --output NEW_DIR --roi 620 230 220 240` | ingestion `frames[]`/HIL `items[]`/thư viện capture `photos[]` versioned; giữ status nguồn; bound hash/byte/kích thước/thời gian/tên; lỗi input đã accepted, mơ hồ, duplicate, traversal, collision, oversize và ROI fail-closed |
| TEST-032 | Replay acquisition thật và cổng holdout chưa gán nhãn | kiểm tra hai capture manifest với 75 JPEG; chạy archive inspector vào `90_TESTS_ANALYSIS/20260905_Phase03_PixelDiagnostics_75frames_v1`; `PYTHONPATH=src; python -m unittest tests.test_phase03.Phase03Tests.test_benchmark_rejects_unlabeled_or_incomplete_ground_truth` | 75/75 khớp byte/hash, 75 `WARNING`/bị loại, 150 overlay, 0 accepted/đủ điều kiện máy; benchmark từ chối input chưa gán nhãn hoặc positive thiếu tâm |
