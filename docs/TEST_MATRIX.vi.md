...
| TEST-031 | Adapter exploratory JPEG thật phía host | `PYTHONPATH=src; python -m unittest tests.test_jpeg_adapter -v`; `python scripts/explore_picture.py <path> --roi X Y WIDTH HEIGHT` cho diagnostics pixel-only ảnh archive, hoặc thêm đủ calibration explicit để chạy hai pipeline | diagnostics pixel bounded luôn; pipeline detector chỉ chạy khi có calibration; ảnh user vẫn chưa gán nhãn/bị loại, không claim metric |
