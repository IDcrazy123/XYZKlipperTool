...
| TEST-031 | Adapter exploratory JPEG thật phía host | `PYTHONPATH=src; python -m unittest tests.test_jpeg_adapter -v`; lệnh explicit `python scripts/explore_picture.py <path> --calibration-id ... --camera-fingerprint ... --source-sha256 ... --roi X Y WIDTH HEIGHT --mm-per-pixel-x ... --mm-per-pixel-y ...` | hai pipeline bounded, diagnostic chất lượng và fault identity/input đạt; ảnh user vẫn chưa gán nhãn/bị loại, không claim metric |
