...
| TEST-028 | Corpus synthetic đã commit Phase 03 | `venv pinned; PYTHONPATH=src; python -m unittest tests.test_phase03.Phase03Tests.test_committed_synthetic_fixture_and_benchmark_artifact_validate` | PGM ba session, hash, label, path tương đối, manifest và report benchmark đều validate |
