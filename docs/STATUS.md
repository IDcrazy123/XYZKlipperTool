...
- Phase state: `NEEDS_WORK` — bounded host OpenCV adapter and archived pixel-candidate inspection are IMPLEMENTED; real sanitized session-separated holdout and calibration/ground truth are absent
...
| 03 Camera and vision pipeline | `NEEDS_WORK` | Bounded host OpenCV adapter and calibration-free candidate reports/overlays implemented; real images remain WARNING/unhomed with no ground truth or calibration, reliability remains unproven, HIL partial run has 1 invalid T0 + 3 excluded LED-on T1 frames and 0 T2 frames; physical behavior `REQUIRES_HIL` |
