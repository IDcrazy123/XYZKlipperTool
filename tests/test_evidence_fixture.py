...
class EvidenceFixtureTests(unittest.TestCase):
    def test_t3_fixture_reproduces_evidence_statistics_without_defaulting_domain(self):
        fixture = json.loads((Path(__file__).parent / "fixtures" / "ktamv-t3-evidence.json").read_text())
        result = summarize([Observation(f"t3-{i}", value) for i, value in enumerate(fixture["x_mm_values"])])
