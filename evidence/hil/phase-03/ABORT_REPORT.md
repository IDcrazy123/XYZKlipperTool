...
The initial T2 LED-off query showed all T0–T4 `color_data` channels exactly zero, but `toolchanger.tool_number=-1` conflicted with `detected_tool_number=2`; capture was therefore rejected fail-closed. The later continuation was also stopped by the usage-limit gate. A later supervisor query on 2026-09-05 observed the printer stationary at displayed X170 Y20 Z40 with T0 declared/detected and heater targets 0, but `homed_axes` was empty. No homing, motion, camera, LED, or Moonraker command is authorized by this report.
